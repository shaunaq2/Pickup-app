...
export default function GameModal({
  ...
  onApprove, onDeny, onClose, onGoToWallet, onUnhost, onOpenChat,
}: Props) {
    ...
    async function load() {
      const [{ data: friendRows }, { data: inviteRows }] = await Promise.all([
        supabase.from("friendships").select("requester, recipient")
          .or(`requester.eq.${username},recipient.eq.${username}`).eq("status", "accepted"),
        supabase.from("game_invites").select("invitee")
          .eq("game_id", game.id).eq("inviter", username),
      ]);
  ...
  async function sendInvite(toUsername: string) {
    ...
    await supabase.from("game_invites").upsert({
      game_id: game.id, inviter: username, invitee: toUsername, status: "pending",
  ...
  function confirmUnhost() { setUnhostConfirm(false); onUnhost?.(); onClose(); }
        ...
        {leaveConfirm ? (
          ...
          <>
            ...
            <div className="modal-section">
              ...
              <div className="detail-row"><span className="detail-label">Date</span><span className="detail-val">{formatDate(game.date)}</span></div>
              <div className="detail-row"><span className="detail-label">Time</span><span className="detail-val">{formatTime(game.time)}</span></div>
              <div className="detail-row"><span className="detail-label">Duration</span><span className="detail-val">{game.duration} min</span></div>
              <div className="detail-row"><span className="detail-label">Skill level</span><span className="detail-val">{SKILL_LABEL[game.skillLevel]}</span></div>
              <div className="detail-row"><span className="detail-label">Availability</span><span className="detail-val">{spotsLeft(game)} of {game.spots} spots open</span></div>
              {game.groundCost > 0 && (
                <div className="detail-row"><span className="detail-label">Ground cost</span><span className="detail-val">${game.groundCost.toFixed(2)} total</span></div>
              )}
              {hasCost && (
                <div className="detail-row"><span className="detail-label">Cost per player</span><span className="detail-val cost-highlight">${game.costPerPlayer.toFixed(2)}</span></div>
              )}
              ...
              {game.recurring && (
                <div className="detail-row"><span className="detail-label">Schedule</span><span className="detail-val" style={{ color: "var(--green)" }}>🔁 Repeats weekly</span></div>
              )}
              {hasWaitlist && (
                <div className="detail-row"><span className="detail-label">Waitlist</span><span className="detail-val">{game.waitlist.length}/{game.waitlistMax} spots</span></div>
              )}
              {game.note && (
                <div className="detail-row"><span className="detail-label">Note</span><span className="detail-val">{game.note}</span></div>
              )}
            </div>
            ...
            {(isHost || joined) && friends.length > 0 && (
              <div className="modal-section">
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }}
                  ...
                  <div className="modal-section-title" style={{ marginBottom: 0 }}>Invite friends</div>
                    ...
                    {friends.map((f) => {
                      ...
                      return (
                        <div key={f.username} style={{
                          ...
                          <div style={{ flex: 1, fontWeight: 600, fontSize: 13, color: "var(--text)" }}>@{f.username}</div>
                          {invited ? (
                            <span style={{ fontSize: 11, color: "var(--green)", fontWeight: 600 }}>✓ Invited</span>
                          ) : (
                            <button onClick={() => sendInvite(f.username)} disabled={inviteLoading === f.username}
                              style={{ padding: "5px 12px", borderRadius: 8, fontSize: 12, border: "none", background: "var(--green)", color: "#fff", cursor: "pointer", fontWeight: 600 }}>
            ...
            {isHost && (
              <div style={{ marginTop: 8 }}>
                {canUnhost && onUnhost ? (
                  <button className="modal-action-btn leave" onClick={() => setUnhostConfirm(true)}>Delete game</button>
                ) : (
            ...
            {!isHost && (
              <>
                {joined ? (
                  withdrawOk ? (
                    <button className="modal-action-btn leave" onClick={() => setLeaveConfirm(true)}>Leave game</button>
                  ) : (
                    <div className="withdraw-locked-block">
                      <div className="withdraw-lock-icon">🔒</div>
                ...
                {!joined && hasCost && !isPrivate && (
                  <div className="modal-credit-row">
                    <span className="modal-credit-balance">Your balance: <strong>${balance.toFixed(2)}</strong></span>
                    {!canAfford && <button className="modal-topup-link" onClick={onGoToWallet}>Add credits →</button>}
                  </div>
                )}
              </>
            )}
...
function LeaveConfirm({ game, hasCost, onConfirm, onCancel }: {
  ...
  return (
    <div className="leave-confirm">
      ...
      <div className="leave-confirm-sub">{game.sport.charAt(0).toUpperCase() + game.sport.slice(1)} at {game.location}</div>
      {hasCost && <div className="leave-confirm-refund"><span className="refund-pill">↩ ${game.costPerPlayer.toFixed(2)} refunded to wallet</span></div>}
      <div className="leave-confirm-actions">
