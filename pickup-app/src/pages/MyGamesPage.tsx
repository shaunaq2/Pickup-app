...
export default function MyGamesPage({
  ...
  if (totalPlaying === 0 && totalHosting === 0) {
    return (
      <div className="empty">
        <div className="empty-icon">📋</div>
        Nothing here yet.<br />Browse games to join one, or post your own.
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="sub-tabs" style={{ marginTop: 10 }}>
        <button className={`sub-tab ${subTab === "upcoming" ? "active" : ""}`} onClick={() => setSubTab("upcoming")}>
          Upcoming
          {topTab === "playing" && playUpcoming.length > 0 && <span className="sub-tab-count">{playUpcoming.length}</span>}
          {topTab === "hosting" && hostUpcoming.length > 0 && <span className="sub-tab-count">{hostUpcoming.length}</span>}
        </button>
        <button className={`sub-tab ${subTab === "past" ? "active" : ""}`} onClick={() => setSubTab("past")}>
          Past
          {topTab === "playing" && (playPast.length + liveHistory.length) > 0 && <span className="sub-tab-count">{playPast.length + liveHistory.length}</span>}
          {topTab === "hosting" && hostPast.length > 0 && <span className="sub-tab-count">{hostPast.length}</span>}
        </button>
      </div>
      ...
      {topTab === "playing" && subTab === "upcoming" && (
        <>
          {playUpcoming.length === 0 ? (
            ...
            <GameCard key={g.id} game={g} joined={joinedIds.has(g.id)} isHost={false}
              requested={requestedIds.has(g.id)} waitlisted={waitlistedIds.has(g.id)}
              balance={balance} onOpen={setModalId}
              onToggleJoin={(id) => {
      ...
      {topTab === "hosting" && subTab === "upcoming" && (
        <>
          {hostUpcoming.length === 0 ? (
            ...
            <HostGameRow key={g.id} game={g} onOpen={setModalId} onOpenChat={setChatGameId} />
          ))}
        </>
      )}
...
function HostGameRow({ game, onOpen, onOpenChat }: { game: Game; onOpen: (id: number) => void; onOpenChat: (id: number) => void }) {
  ...
  return (
    <div className="host-game-row" onClick={() => onOpen(game.id)}>
      ...
      <div className="host-game-meta">
        <div className="meta-row"><CalIcon />{formatDate(game.date)} · {formatTime(game.time)} · {game.duration} min</div>
        <div className="meta-row"><PinIcon />{game.location}, {game.city}</div>
      </div>
      <div className="host-game-footer">
        <div className="host-game-players">
          ...
          {game.players.length > 5 && <div className="stacked-more">+{game.players.length - 5}</div>}
        </div>
        {pending > 0 && <span className="pending-badge">{pending} pending</span>}
        <button onClick={(e) => { e.stopPropagation(); onOpenChat(game.id); }}
          style={{
...
function PastRecordRow({ record }: { record: BookingRecord }) {
  ...
  return (
    <div className="past-game-row">
      <div className="past-game-icon" style={{ background: sport?.bg ?? "#eee" }}>{sport?.icon ?? "🏅"}</div>
      <div className="past-game-body">
        <div className="past-game-name">{sport?.label ?? record.sport}</div>
        <div className="past-game-meta">{record.location} · {days[record.dayOfWeek]}s · {h}:00 {ampm}</div>
      </div>
...
function CalIcon() {
  return <svg className="meta-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="2" y="3" width="12" height="11" rx="1.5" /><path d="M5 1.5v3M11 1.5v3M2 7h12" /></svg>;
}
function PinIcon() {
  return <svg className="meta-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M8 1.5C5.5 1.5 3.5 3.5 3.5 6c0 3.5 4.5 8.5 4.5 8.5s4.5-5 4.5-8.5c0-2.5-2-4.5-4.5-4.5z" /><circle cx="8" cy="6" r="1.5" /></svg>;
}
